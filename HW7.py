...
def make_players_table(data, cur, conn):
    ...
    for player in data['squad']:
        ...
        for row in cur:
            pos = row[0]
            cur.execute("INSERT INTO Players (id, name, position_id, birthyear, nationality) VALUES (?, ?, ?, ?, ?)", (player['id'], player['name'], pos, int(player['dateOfBirth'][:4]), player['nationality']))
    conn.commit()
...
def birthyear_nationality_search(age, country, cur, conn):
    list_tup = []
    start_year = 2023 - age
    cur.execute("SELECT name, nationality, birthyear FROM Players WHERE (birthyear < ?) AND (nationality = ?)", (start_year, country))
    for row in cur:
        list_tup.append(row)
    return list_tup
...
def position_birth_search(position, age, cur, conn):
    list_tup = []
    start_year = 2023 - age
    cur.execute("SELECT name, position, birthyear FROM Players JOIN Positions ON position_id = Positions.id WHERE (birthyear > ?) AND (position = ?)", (start_year, position))
    for row in cur:
        list_tup.append(row)
    return list_tup
